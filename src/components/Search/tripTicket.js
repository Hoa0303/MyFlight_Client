...
const TripTicket = () => {
    ...
    const [tripRoute, setRoute] = useState([]);
            ...
            const fetchData = async () => {
                try {
                    ...
                    const flight = allData.flights.filter(e => {
                        return (
                            e.SanBayDi === departurePoint[0].name &&
                            e.SanBayDen === destination[0].name
                        );
                    });
                    ...
                    const route = allData.routes.filter(e => {
                        return (
                            ...
                            e.NgayKhoiHanh === formData?.calendarValue?.[0]
                        );
                    });
                    setRoute(route);
                } catch (error) {
    ...
    const handleSelectTicket = (routeId) => {
        ...
        Cookies.set('IdTripRoute', routeId);

        // Lưu selectedTicketType vào cookies
        Cookies.set('TripType', selectedTicketType);
                    ...
                    {tripRoute.map(route => (
                        <div key={route._id} className="row text-white mb-5 custom-border">
                            ...
                            <div className="col-lg-3  col-md-4 col-sm-4 col-6 mt-3 mb-3">
                                ...
                                <p>Khởi hành: {formData.departurePoint}</p>
                            </div>
                            <div className="col-lg-3  col-md-4 col-sm-4 col-6 mt-3 mb-3">
                                <p>{route.NgayKhoiHanh}</p>
                                <h3> {route.GioDen}</h3>
